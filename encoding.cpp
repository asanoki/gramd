...
size_t importAsUtf8(wchar_t *output, const char *input, size_t output_size, size_t input_size) {
	const char *pc = 0;
	wchar_t *pwc = 0;
	std::codecvt<wchar_t, char, mbstate_t>::result result;
	std::mbstate_t state = std::mbstate_t();
	boost::archive::detail::utf8_codecvt_facet *codec_ptr = utf8_codec.get();
	result = codec_ptr->in(state, input, input + input_size, pc, output, output + output_size, pwc);
...
size_t exportAsUtf8(char *output, const wchar_t *input, size_t output_size, size_t input_size) {
	const wchar_t *pwc = 0;
	char *pc = 0;
	std::codecvt<wchar_t, char, mbstate_t>::result result;
	std::mbstate_t state = std::mbstate_t();
	boost::archive::detail::utf8_codecvt_facet *codec_ptr = utf8_codec.get();
	result = codec_ptr->out(state, input, input + input_size, pwc, output, output + output_size, pc);
	if (result != (std::codecvt<wchar_t, char, mbstate_t>::ok)) {
		throw ConversionException();
	}
	size_t result_size = 0;
